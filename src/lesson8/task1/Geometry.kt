...
fun diameter(vararg points: Point): Segment {
    ...
    var maxSegmentLength = Segment(Point(0.0, 0.0), Point(0.0, 0.0))
    if (points.size < 2) throw IllegalArgumentException()
    for (i in 0 until points.size)
        for (j in (i + 1) until points.size) {
            val nextSegment = points[i].distance(points[j])
            if (nextSegment > previousSegment) {
                maxSegmentLength = Segment(points[i], points[j])
                previousSegment = points[i].distance(points[j])
            }
        }
